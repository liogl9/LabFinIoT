...
AUX_EVENTHUB_SAS = IOT_HUB_BUILT_IN_ENDPOINT_CONNECTION_STRING.split(";")[
    2][16:]
AUX_IOT_HUB_CONNECTION_STRING = "HostName=icaiiotlabgroup01Lio.azure-devices.net;SharedAccessKeyName=iothubowner;SharedAccessKey={}".format(
    AUX_EVENTHUB_SAS)
...
def on_iot_hub_message_event(partition_context, event):
    ...
    if event_temperature > MAXIMUM_TEMPERATURE:
        ...
        command_to_device_message_body.join(
            'ONE ACTION CODE YOU SHOULD IMPLEMENT')
    ...
    if event_lux['0'] < MIN_LUX:
        send = 1
        print("Lux {} from device {} is lower than {}".format(
            event_lux, event_device_name, MIN_LUX))
        print("Sending alert to device {}".format(event_device_name))

        # MESSAGES TO DEVICES HAVE A BODY
        command_to_device_message_body = "Algo va mal"
        # MESSAGES TO DEVICES HAVE A DICTIONARY OF PROPERTIES
        command_to_device_message_properties['lux_command'] = 'Ondev0'
