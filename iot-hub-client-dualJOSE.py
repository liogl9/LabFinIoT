...
def get_sensor_temperature():
    temperature = AUX_BASE_BODY_TEMPERATURE + \
...
def message_listener(client):
    while True:
        ...
        if 'Lux_cmd' in message.custom_properties:
            print(message.custom_properties['Lux_cmd'])
            ...
            #     inst = 1
            #     msg = "%s" % (inst)
            #     clientM.publish("IIoT/4/relay/0/set".formar, msg)
            #     time.sleep(5)
            #     clientM.disconnect()

        # if message.data.decode("utf-8") == 'off':
        ...
        #     inst = 0
        #     msg = "%s" % (inst)
        #     clientM.publish("IIoT/4/relay/0/set", msg)
        #     time.sleep(5)
        #     clientM.disconnect()
